...
class View:
    ...
    def print_room(self, room, char):
        ...
        if room.is_exit():
            self.window.blit(pygame.transform.scale(self.ceiling_tiles[8 + orientation%4], (size_width, size_height)), (0, 0))
        else:
    ...
    def print_numbers(self, number, is_percent, width, height, posx, posy):
        ...
        for i in range(0, len(letters)):
            if is_percent:
                self.window.blit(pygame.transform.scale(self.numbers_tiles[letters[len(letters)-1-i]], (int(width), int(height))), (int(posx - width * (i+2)), int(posy)))
            else:
                self.window.blit(pygame.transform.scale(self.numbers_tiles[letters[i]], (int(width), int(height))), (int(posx + width*i), int(posy)))
        if is_percent:
            self.window.blit(pygame.transform.scale(self.numbers_tiles[10], (int(width), int(height))), (int(posx - width), int(posy)))
    ...
    def print_fillers(self, char = None):
        ...
        self.window.blit(pygame.transform.scale(self.fills_main[char.get_orientation() % 4], (int(size_width_main), int(size_height_main))), (0, int(starty + self.wheight * 0.01)))
        ...
        for i in range(0, 3):
            self.print_numbers(percentages[i]*100, True, size_width_number, size_height_number, size_width_main + size_width_tubes, starty + self.wheight * 0.01 + size_height_main / 16 + (5 * size_height_main / 16) * i + size_height_main / 32)
        # Numbers Plain
        for i in range(0, 3):
            self.print_numbers(plain_numbers[i], False, size_width_number, size_height_number, size_width_main, starty + self.wheight * 0.01 + size_height_main / 16 + (5 * size_height_main / 16) * i + size_height_main / 32)
